...
class SoftActorCritic(nn.Module):
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor) -> torch.Tensor:
        ...
        if self.target_critic_backup_type == "doubleq":
            if self.num_critic_networks != 2:
                raise ValueError(
                    f"doubleq backup requires exactly 2 critics, got {self.num_critic_networks}"
                )
            next_qs = next_qs.flip(0)
        elif self.target_critic_backup_type in ["min", "clipq", "clipped", "clipped_doubleq"]:
            if self.num_critic_networks < 2:
                raise ValueError(
                    f"min/clipped backup requires at least 2 critics, got {self.num_critic_networks}"
                )
            next_qs = torch.min(next_qs, dim=0).values
        elif self.target_critic_backup_type == "redq":
            raise NotImplementedError("redq backup is not implemented.")
        else:
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            if self.use_entropy_bonus and self.backup_entropy:
                ...
                backup = next_qs + self.temperature * next_action_entropy[None, :]
            else:
                backup = next_qs
            ...
            target_values: torch.Tensor = (
                reward[None, :]
                + self.discount * (1 - done[None, :]) * backup
            ).detach()
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            obs_expanded = obs[None, ...].expand(self.num_actor_samples, *obs.shape)
    ...
    def update(
        ...
        actor_info = {}
        if self.actor_gradient_type in ["reparametrize", "reinforce"]:
